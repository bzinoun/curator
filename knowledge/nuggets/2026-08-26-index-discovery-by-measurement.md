...
Found here: `FPTCONTR(INTMEDC)` was queried on 100k rows in every search-by-agent, but never indexed. The planner used a composite PK for ordering but read all rows before filtering. Adding the index dropped cost by 6× and made it **constant**, not linear.
